...
class TelegramNotifier:
    ...
    def format_news_alert_card(self, item: Dict[str, Any]) -> str:
        """Format a clean, institutional-grade Telegram alert card for stock news."""
        from datetime import datetime, timezone, timedelta
        from email.utils import parsedate_to_datetime

        company = html.escape(str(item.get("company_name") or "Watchlist Stock"))
        symbol = html.escape(str(item.get("symbol") or ""))
        source = html.escape(str(item.get("source") or "Financial Media"))
        title = sanitize_for_telegram_html(str(item.get("title") or ""))
        catalyst = html.escape(str(item.get("catalyst") or "⚡ BREAKING MARKET UPDATE"))
        catalyst_desc = sanitize_for_telegram_html(str(item.get("catalyst_desc") or ""))
        reporter = html.escape(str(item.get("reporter") or ""))
        link = str(item.get("link") or "")
        raw_pub = str(item.get("published_time") or "")

        # Format publication time in IST
        time_display = raw_pub
        try:
            ...
            ampm = "AM" if hour < 12 else "PM"
            h12 = hour % 12 or 12
            time_part = f"{h12:02d}:{minute:02d} {ampm} IST"
            if dt_ist.date() == today_ist:
                time_display = f"Today, {time_part}"
            elif dt_ist.date() == yesterday_ist:
                time_display = f"Yesterday, {time_part}"
            else:
                time_display = f"{dt_ist.strftime('%d %b %Y')}, {time_part}"
        except Exception:
            time_display = raw_pub
        time_str = html.escape(time_display)
        ...
        lines = [
            ...
            f"🏢 <b>{company.upper()}</b>",
            f"📌 <b>Ticker:</b> <code>{symbol}</code> (NSE/BSE) | 🕒 {time_str}",
            f"🏛️ <b>Source:</b> <b>{source}</b>",
            f"📊 <b>Catalyst:</b> {catalyst}",
        ]

        if reporter:
            lines.append(f"🎙️ <b>Anchor / Reporter:</b> <b>{reporter}</b>")
        ...
        lines.extend([
            ...
            f"• <b>Type:</b> {catalyst}",
            f"• <b>Context:</b> {catalyst_desc}",
        ])

        if link and link.startswith("http"):
            lines.extend([
                "",
                f"🔗 <a href=\"{link}\"><b>Read Full Story on {source}</b></a>"
            ])

        lines.append("━━━━━━━━━━━━━━━━━━━━━━")
        return "\n".join(lines)

    def dispatch_news(self, item: Dict[str, Any]) -> str:
        """Format news alert card and broadcast to all Telegram destinations."""
        card = self.format_news_alert_card(item)
