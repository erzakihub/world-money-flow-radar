...
export default function AssetBullRunRadar() {
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch assets rankings data", e);
    }
    setLoading(false);
  };
  ...
  const scatterData = rankings.map(asset => {
    // Deduce price confirmation and liquidity support from asset scores
    let priceConf = 50;
    let liqSupp = 50;
    ...
    if (asset.id === "gold") {
      ...
      priceConf = 80;
      liqSupp = 58;
    } else if (asset.id === "china_equities") {
      priceConf = 42;
      liqSupp = 55;
    } else {
        ...
        <div className="bg-gray-950 border border-gray-800 p-3 rounded-lg shadow-xl text-xs font-sans">
          ...
          <div className="space-y-1 text-gray-400 font-mono">
            ...
            <p>Quadrant: <span className="text-brand-blue font-semibold">
              {d.y >= 50 && d.x >= 50 ? "Bull Pocket" : 
               d.y >= 50 && d.x < 50 ? "Accumulation" : 
               d.y < 50 && d.x >= 50 ? "Divergence Risk" : "Avoid / Outflow"}
            </span></p>
          </div>
        </div>
      );
  ...
  // Group assets by category for heatmap
  ...
  const getHeatmapColor = (score: number) => {
    ...
    if (score >= 30) return "bg-orange-500/10 text-orange-400 border-orange-500/20";
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-heading font-bold text-white tracking-tight flex items-center gap-2">
            ...
            <span>Asset Bull Run Radar</span>
          </h2>
          <p className="text-xs text-gray-500 mt-0.5">
            Aggregates flow variables, relative strength (RRG), rates, and currencies to discover bull run entry pockets.
          </p>
        </div>
        ...
        <div className="px-3 py-1 bg-gray-900 border border-gray-800 rounded-lg flex items-center gap-2 text-xs font-mono text-gray-400">
          ...
          <span>Ingested Channels: 25 Global Markets</span>
        </div>
      </div>
      ...
      <div className="bg-bg-card border border-gray-850 rounded-xl p-5 shadow-md space-y-4">
        <h3 className="text-xs font-mono text-gray-500 uppercase tracking-wider">
          Asset Class Money Flow Heatmap
        </h3>
      ...
      {/* Main Grid: Scatter plot & Rankings (Left 8 cols) vs Asset detail (Right 4 cols) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* Left Column (8 cols) */}
        <div className="lg:col-span-8 space-y-6">
          
          {/* 1. 2D Scatter Quadrant */}
          <div className="bg-bg-card border border-gray-850 rounded-xl p-5 shadow-md flex flex-col justify-between relative overflow-hidden">
            <h3 className="text-xs font-mono text-gray-500 uppercase tracking-wider mb-4">
              Money Flow Quadrant Matrix (Liquidity vs Price)
            </h3>
            ...
            <div className="h-[320px] relative">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 10, right: 20, left: -20, bottom: 5 }}>
                  <XAxis 
                    ...
                    tickLine={false}
                  >
                    <Label value="Price Strength / Relative Momentum ➜" offset={-5} position="insideBottom" fill="#6b7280" fontSize={10} />
                  </XAxis>
              ...
              {/* Corner Labels */}
              <div className="absolute top-2 left-10 text-[9px] font-mono text-gray-500 bg-gray-900/60 px-1.5 py-0.5 rounded border border-gray-800/40">
                ACCUMULATION (Low Price, High Liquidity)
              </div>
              <div className="absolute top-2 right-4 text-[9px] font-mono text-brand-green bg-gray-900/60 px-1.5 py-0.5 rounded border border-gray-800/40">
                BULL POCKET (High Price, High Liquidity)
              </div>
              <div className="absolute bottom-10 left-10 text-[9px] font-mono text-brand-red bg-gray-900/60 px-1.5 py-0.5 rounded border border-gray-800/40">
                AVOID / OUTFLOW (Low Price, Low Liquidity)
              </div>
              <div className="absolute bottom-10 right-4 text-[9px] font-mono text-brand-yellow bg-gray-900/60 px-1.5 py-0.5 rounded border border-gray-800/40">
          ...
          <div className="bg-bg-card border border-gray-850 rounded-xl overflow-hidden shadow-md">
            <div className="p-5 border-b border-gray-800/60 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <input 
                  ...
                  className="bg-transparent border-none text-xs text-white focus:outline-none focus:ring-0 placeholder-gray-600 w-44 md:w-64"
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <select 
                  ...
                  className="bg-gray-900 border border-gray-800 text-[10px] font-mono text-gray-400 rounded px-2 py-1 focus:outline-none"
                >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs font-sans">
                <thead>
                  <tr className="border-b border-gray-800 text-gray-500 font-mono bg-gray-900/10">
                    ...
                    <th className="py-3 px-4 font-semibold text-[10px] uppercase text-right">Signal</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-900">
                  {filteredRankings.map((asset, index) => {
                    const isSelected = selectedAsset?.id === asset.id;
                    const rank = rankings.findIndex(item => item.id === asset.id) + 1;
                    return (
                      <tr 
                        key={asset.id}
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-right">
                          <span className="font-mono font-bold text-white text-sm">{asset.score.toFixed(1)}</span>
                        </td>
                        ...
                        <td className={`py-3 px-4 text-center font-mono font-semibold ${asset.change_3m >= 0 ? "text-brand-green" : "text-brand-red"}`}>
                          {asset.change_3m > 0 ? "+" : ""}{asset.change_3m.toFixed(1)}%
                        </td>
                        <td className="py-3 px-4 text-right">
                          <BullRunSignalBadge signal={asset.signal} />
                        </td>
                      </tr>
                    );
                  })}
        ...
        {/* Right Column: Asset Detail Drawer (4 cols) */}
        <div className="lg:col-span-4">
          {selectedAsset ? (
            <div className="bg-bg-card border border-gray-850 rounded-xl p-5 space-y-5 sticky top-24 shadow-xl flex flex-col justify-between">
              ...
              <div className="grid grid-cols-2 gap-4 bg-gray-950/40 p-4 rounded-xl border border-gray-900">
                <div className="flex flex-col">
                  <span className="text-[9px] font-mono text-gray-500 uppercase">Bull Run Score</span>
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="mt-1">
                    <BullRunSignalBadge signal={selectedAsset.signal} />
                  </div>
                </div>
              </div>
              ...
              {historyLoading ? (
                <div className="h-[120px] flex items-center justify-center border border-gray-850/50 rounded-lg bg-gray-955/10">
                  <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-brand-green"></div>
                </div>
              ) : (
                <div className="space-y-1.5">
                  <span className="text-[9px] font-mono text-gray-500 uppercase">Historical Score Trend (12 Months)</span>
              ...
              <div className="bg-gray-955/40 p-3.5 rounded-xl border border-gray-900 text-xs space-y-2.5 font-sans leading-relaxed">
                ...
                <div>
                  <span className="text-[8px] font-mono text-brand-red uppercase block">Opposite / Invalidation Risk</span>
              ...
              <div className="flex justify-between items-center text-[10px] font-mono text-gray-500 border-t border-gray-900 pt-3">
                <span>Confidence Index: <strong className="text-white">{selectedAsset.confidence_score}%</strong></span>
            ...
            <div className="bg-bg-card border border-gray-850 rounded-xl p-6 text-center h-full flex flex-col items-center justify-center text-gray-500 min-h-[300px]">
              ...
              <p className="text-xs">
                Select any asset class in the rankings table or click points in the quadrant matrix to load the macro explainability details.
              </p>
            </div>
          )}
        </div>
